...
export function applySorting(
  ...
  for (const rule of config.rules) {
    ...
    if (rule.type === 'simple' && rule.simple) {
      ...
      // Log first 5 results
      console.log('First 5 results after linked-issues sort:', results.slice(0, 5).map((r, i) => ({
        position: i + 1,
        key: r.row[rule.linkedIssues!.keyField],
        linkedIssues: r.row[rule.linkedIssues!.linkedIssuesField],
        groupId: r.groupId,
  ...
  // TODO: Apply AI sorting if enabled
  // if (config.aiSort?.enabled) {
  //   results = await applyAISorting(results, config.aiSort)
  // }
