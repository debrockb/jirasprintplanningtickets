...
- **dataStore.ts** - Core application state: ticket data rows, field mappings, field layouts (grid positions), field styles, enrichment groups, card background rules. Contains `getEnrichedRow()` helper that merges enrichment data with base row.
- **templateStore.ts** - Persists templates and enrichment configurations to localStorage. Handles export/import as JSON.
...
- **CardDesigner.tsx** - Main canvas using react-grid-layout for drag-drop field placement with inline style controls
...
- **PrintView.tsx** - Print preview with half-A4 and full-A4 size options
...
Key interfaces: `TicketRow`, `FieldMapping`, `FieldLayout`, `FieldStyle`, `ColorRule`, `CardBackgroundRule`, `EnrichmentGroup`, `CardTemplate`, `AIProvider`
...
2. Templates loaded via TemplateManager → smart column mapping adapts to new data structure
3. AI processing: AIFieldProcessor → `aiService.sendChatMessage()` → provider → `updateAllRowsField()` or `updateRowField()`
