...
export function TemplateManager() {
  ...
  const enrichments = useTemplateStore(state => state.enrichments)
  ...
  const handleSaveTemplate = async () => {
    ...
    const newTemplate = {
      ...
      fieldLayouts,
      fieldStyles,
      cardBackgroundRules: cardBackgroundRules || []
    }
  ...
  const handleSaveEnrichment = async () => {
    ...
    const newEnrichment = {
      id: Date.now().toString() + Math.random().toString(36).substring(2, 9),
      name: enrichmentName.trim(),
      createdAt: Date.now(),
      enrichmentGroup
    }

    await saveEnrichment(newEnrichment)
    setEnrichmentName('')
  }

  const handleLoadEnrichment = (id: string) => {
    const saved = enrichments.find(e => e.id === id)
    if (saved) {
